...
ST_pts = 2001  # num points to sweep over ST
SLB_pts = 201

ST_gate_range = np.linspace(3.45, 3.5, ST_pts)
SLB_gate_range = np.linspace(0.87, 0.88, SLB_pts)
...
experiment = {
    "desc":         "Sweeping a zoomed in region of the gate voltages of the Si CMOS to tune the SET",
    "ST":           f"range from 3.45v to 3.5v, {ST_pts}pts",
    "SLB and SRB":  f"0.87v to 0.88v, {SLB_pts}pts"
    }

monty = Monty("SET_long_2d_sweep_zoomed", experiment)
...
with tqdm(total=ST_pts*SLB_pts) as pbar:
    for (j, SLB_SRB_voltage) in enumerate(SLB_gate_range):
        ...
        for (i, ST_voltage) in enumerate(ST_gate_range):
            si.ST(ST_voltage)
            time.sleep(0.05)  # wait for the MDAC to update
