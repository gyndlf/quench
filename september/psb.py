...
# Relative imports to keep compatibility
...
def setup_hyper_sequencers(shfqc: SHFQC, params, print_programs=False):
    ...
    ramp_rate = params["amplitude_volts"]["ramp_rate"]
    # Create mixed state 
    pwr = params["powers"]["drive"]
    maxv = max_volt(pwr)  # use this instead of 0.34 to generalise
    samples = timeToSamples(init_len, samplingDivider)
    #mixed_p = np.linspace(volt_to_arbitrary(0.34 - 0.34 * samples * ramp_rate, 0), volt_to_arbitrary(0.34, 0), samples) # ramps
    mixed_p = np.linspace(volt_to_arbitrary(maxv, pwr), volt_to_arbitrary(maxv, pwr), samples)  # no ramping
    # Read pulse driver
    samples = timeToSamples(read_len, samplingDivider)
    read_p = np.linspace(volt_to_arbitrary(maxv - maxv*samples*ramp_rate, pwr), volt_to_arbitrary(maxv, pwr), samples)
